...
namespace WebApplication1.Controllers
{
    public class LiveCattleController : Controller
    {
        ...
        ///Add
        [Route("PostlcattleDataData")]
        [HttpPost]
        public async Task<IEnumerable<LiveCattleHistory>> PostlcattleDataData()
        {
            List<LiveCattleHistory> hist = new List<LiveCattleHistory>();
            hist = await _dbContext.liveCattleHistories.ToListAsync();
            int LastID = _dbContext.liveCattleHistories.Max(p => p.cattle_Id);


            var web = new HtmlWeb();
            var doc = web.Load("https://www.investing.com/commodities/live-cattle-historical-data");


            var historyNode = doc.DocumentNode.SelectNodes("//tbody/tr[1]");

            var HData = new List<LiveCattleHistory>();
            ...
            if (historyNode != null)
            {
                foreach (var cryptoNode in historyNode)
                {
                    ...
                    {
                        HData.Add(new LiveCattleHistory
                        {
                            Date = d,
                            Price = p,
                            Open = o,
                        ...
                        if (_dbContext.liveCattleHistories.Any(o => o.Date.Contains(date)))
                            throw new Exception("Record Already exists! TRY ADDING TOMMORROW ;)");
                    }
                }
                try
                {
                    foreach (LiveCattleHistory pc in HData)
                    {
                        await _dbContext.liveCattleHistories.AddAsync(pc);
                    }
                }
                catch
                {
